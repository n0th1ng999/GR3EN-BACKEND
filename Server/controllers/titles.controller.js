...
module.exports={
    getTitles: (req,res) =>{
        ...
        if(length && offset){
            ...
            Title.find().skip(offset).limit(length).then(titles => {res.status(206).json(titles)}).catch(err => {res.status(400).send({err: err.message})})
    ...
    createTitle:(req,res) =>{
        ...
        .then((title) => {res.status(201).send(title)})
        .catch(err => {res.status(400).send({err:err.message})})
    },
    ...
    deleteTitle:(req,res) => {
        ...
        .then(title =>{
            if(title.deletedCount){
                ...
                res.status(404).send({err:'Deletion not successful'})
            }
        })
        .catch(err => {res.status(400).send({err:err.message})})
    }
